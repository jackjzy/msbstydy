...
public class EqbB2cUtil {
    //此示例为企业和个人场景的签署示例代码，签署方式为一步发起签署，如果需要分步签署，签署部分代码示例可参考b2bDemo
    static{
        String bucketName = CosCofig.getBucketName();
...
//        模拟环境（沙箱环境）域名：https://smlopenapi.esign.cn
        String host= eqbConfig.getHost(); //网关
        String project_id=eqbConfig.getProjectId();//应用id
        String project_scert=eqbConfig.getProjectScert();//密钥
      ...
      public static FfwyContract createContract(FfwyContract contractd){
          FfwyContract contract = new FfwyContract();
          Logger logger = LoggerFactory.getLogger(EqbB2cUtil.class);
          try{
             String filePath="E:\\wemem\\webffwy\\files\\test.pdf";//文件地址
              //String filePath=contractd.getFilePhas();
              //-----------------------个人账号信息用于创建个人账号接口传入-----------------------------
             // String thirdPartyUserIdPsn=contractd.getThirdPartyUserIdPsn();
              String thirdPartyUserIdPsn="12321332321";//thirdPartyUserId参数，用户唯一标识，自定义保持唯一即可
          //    String namePsn=contractd.getNamePsn();//name参数，姓名

              String namePsn="杨志杰";

              String idTypePsn="CRED_PSN_CH_IDCARD";//idType参数，证件类型
             // String idTypePsn=contractd.getIdTypePsn();
              String idNumberPsn="420902199707115954";//idNumber参数，证件号
             // String idNumberPsn=contractd.getIdNumberPsn();//idNumber参数，证件号

              String mobilePsn="17671850175";//mobile参数，手机号
              //String mobilePsn=contractd.getMobilePsn();
              //------------------------企业账号信息用于创建机构账号接口传入----------------
              String thirdPartyUserIdOrg="91110105MA0038GH6Y";//thirdPartyUserId参数，企业唯一标识，自定义保持唯一即可
             // String thirdPartyUserIdOrg=contractd.getThirdPartyUserIdOrg();
              String nameOrg="北京嗅美科技有限公司";//name参数，机构名称
              //String nameOrg=contractd.getNameOrg();
              String idTypeOrg="CRED_ORG_USCC";//idType参数，证件类型
              //String idTypeOrg=contractd.getIdTypeOrg();
              String idNumberOrg="91110105MA0038GH6Y";//idNumber参数,机构证件号
              //String idNumberOrg=contractd.getIdNumberOrg();
              //创建个人账号
              ...
              contract.setPsnSealId(psnSealId);
              ...
              contract.setOrgSealId(orgSealId);
              ...
              contract.setUploadUrl(uploadUrl);
              ...
              orgSignfields.add(new Signfield()
                      .setAutoExecute(true)//自动签署必须为true
                      .setFileId(fileId)
              ...
              String url = fileSignUrlResp.getData().getUrl();
              contract.setUrl(url);
              contract.setShorturl(shortUrl);
